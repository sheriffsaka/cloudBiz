      ...
      <table className="w-full text-sm text-left text-gray-500">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50">
          <tr>
            ...
            <th scope="col" className="px-6 py-3 cursor-pointer" onClick={() => onSort('issueDate')}>Payment Date{getSortIcon('issueDate')}</th> {/* Using issueDate as payment date for receipts */}
          ...
          {invoices.map((invoice) => (
            <tr key={invoice.id} className="bg-white border-b hover:bg-gray-50">
              ...
              <td className="px-6 py-4 text-right">
                <a href="#" onClick={(e) => { e.preventDefault(); onViewInvoice(invoice.id); }} className="font-medium text-primary-600 hover:underline">View Invoice</a>
              </td>
            </tr>
          ))}
           {invoices.length === 0 && (
...
const SentReceiptsList: React.FC<SentReceiptsListProps> = ({ invoices, clients, onViewInvoice }) => {
